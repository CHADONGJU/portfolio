// 매매 원장의 정합성 계산 — 이 앱에서 돈에 관한 가장 중요한 파일.
//
// 매수·매도 행을 시간순으로 훑어 이동평균 평단가, 남은 수량, 실제 투입 원화(원가),
// 그리고 매도마다의 실현손익을 만든다. 규칙 세 가지:
//
// 1) 회차(round)가 다르면 절대 섞지 않는다. 전량 매도 후 재매수한 물량은 별개다.
// 2) 원화 원가는 매수 시점 환율로 쌓는다. 오늘 환율로 환산하면 과거 원금이 매일 흔들린다.
