// 연도별 수익률.
//
// 수익률 = (그 해 실현손익 + 선택 시 수령 배당) ÷ (연초 보유원가 + 연중 매수금액).
// 분모는 "그 해에 실제로 굴린 돈"이고, 재투자한 매수도 다시 더해진다. 평가손익은
// 넣지 않는다 — 아직 확정되지 않은 값이라 해가 바뀔 때마다 과거 수익률이 흔들린다.
//
// 매수원가를 모르는 구간이 하나라도 있으면 숫자를 지어내지 않고 사유와 함께
// null을 돌려준다. 화면은 그 사유를 사용자에게 그대로 보여준다.
