...
test('해외 손익을 주가 몫과 환율 몫으로 나눈다', () => {
  // 주가는 $100 그대로고 환율만 1300 → 1500으로 올랐다.
  // 주가 기여는 0이고, 원화 손익 20만원은 전부 환율에서 왔다.
  const metrics = runHook({
    ...baseOptions,
    tradeLedger: [{
      id: 'b1', name: 'SOXL', ticker: 'SOXL', category: '해외주식', currency: 'USD',
  ...
  assert.equal(metrics.fxContribution.priceKRW, 0);
  assert.equal(metrics.fxContribution.fxKRW, 200000);
  assert.equal(metrics.fxContribution.totalKRW, 200000);
});

test('국내 종목은 환율 기여 계산에 들어가지 않는다', () => {
  const metrics = runHook({
    ...baseOptions,
    assets: [{
      id: 1, name: '삼성전자', ticker: '005930', category: '국내주식', currency: 'KRW',
      quantity: 10, averagePrice: 70000, originalAveragePrice: 70000,
      originalCurrentPrice: 90000, buyDate: '2026-01-05',
    }],
  });

  assert.equal(metrics.fxContribution.totalKRW, 0);
  assert.equal(metrics.fxContribution.fxKRW, 0);
});

test('연환산 수익률은 매수·매도·현재 평가금액을 현금흐름으로 본다', () => {
  // 1년 전 100만원어치를 사서 지금 평가금액이 110만원이면 연 10%다.
  const metrics = runHook({
    ...baseOptions,
    exchangeRate: 1,
    tradeLedger: [{
      id: 'b1', name: '삼성전자', ticker: '005930', category: '국내주식', currency: 'KRW',
      side: 'buy', date: '2025-09-12', quantity: 10, price: 100000,
    }],
    assets: [{
      id: 1, name: '삼성전자', ticker: '005930', category: '국내주식', currency: 'KRW',
      quantity: 10, averagePrice: 100000, originalAveragePrice: 100000,
      originalCurrentPrice: 110000, buyDate: '2025-09-12',
    }],
  });

  assert.ok(metrics.annualizedReturnPercent !== null);
  assert.ok(
    Math.abs(metrics.annualizedReturnPercent - 10) < 0.5,
    `연 10% 근처여야 한다 (${metrics.annualizedReturnPercent}%)`,
  );
});

test('기록이 없으면 연환산 수익률을 지어내지 않는다', () => {
  const metrics = runHook(baseOptions);
  assert.equal(metrics.annualizedReturnPercent, null);
});
